...
def rf_dis(n_trees, X,Y,train_indices,test_indices,seed,md):
    clf = RandomForestClassifier(n_estimators=n_trees,
                                 random_state=seed, oob_score=True, n_jobs=-1,max_depth=md)
...
def mcode(ite):
    ...
    for  i in range(50):
        ...
        erf = []
        se = 1000
        for train_in, test_in in loo.split(dataX):
            se = se+1
